import project_image_1 from "./img/project_image_1.png";
import project_image_2 from "./img/project_image_2.png";
import project_image_3 from "./img/project_image_3.png";
import project_image_4 from "./img/project_image_4.png";

import slide1 from "./img/Projects gallery/Rectangle 23.png"
import slide2 from "./img/Projects gallery/Rectangle 24.png"
import slide3 from "./img/Projects gallery/Rectangle 25.png"
import slide4 from "./img/Projects gallery/Rectangle 26.png"
import slide5 from "./img/Projects gallery/Rectangle 27.png"
import slide6 from "./img/Projects gallery/Rectangle 28.png"
import slide7 from "./img/Projects gallery/Rectangle 29.png"
import slide8 from "./img/Projects gallery/Rectangle 30.png"
import intro from "./img/Intro.png"
import intro2 from "./img/215112.jpg"

export const dataForTest = {
    projects: [
        {
            title: "Parking for 500 auto",
            short_description: "Lorem Ipsum is simply dummy" +
                " text of the printing and typeset" +
                "ting industry. Lorem Ipsum has been the industry's standard dummy t" +
                "ext ever since the 1500s, when an unknown printer took a galley of type" +
                " and scrambled it to make a type specimen book.",
            titleImg: project_image_1
        },
        {
            title: "High School",
            short_description: "Lorem Ipsum is simply dummy" +
                " text of the printing and typeset" +
                "ting industry. Lorem Ipsum has been the industry's standard dummy t" +
                "ext ever since the 1500s, when an unknown printer took a galley of type" +
                " and scrambled it to make a type specimen book.",
            titleImg: project_image_2
        },
        {
            title: "Project Veneer by Jak Fresco",
            short_description: "Lorem Ipsum is simply dummy" +
                " text of the printing and typeset" +
                "ting industry. Lorem Ipsum has been the industry's standard dummy t" +
                "ext ever since the 1500s, when an unknown printer took a galley of type" +
                " and scrambled it to make a type specimen book.",
            titleImg: project_image_3
        },
        {
            title: "lorderon",
            short_description: "Lorem Ipsum is simply dummy" +
                " text of the printing and typeset" +
                "ting industry. Lorem Ipsum has been the industry's standard dummy t" +
                "ext ever since the 1500s, when an unknown printer took a galley of type" +
                " and scrambled it to make a type specimen book.",
            titleImg: project_image_4
        },
        {
            title: "biggest gay building ever",
            short_description: "Lorem Ipsum is simply dummy" +
                " text of the printing and typeset" +
                "ting industry. Lorem Ipsum has been the industry's standard dummy t" +
                "ext ever since the 1500s, when an unknown printer took a galley of type" +
                " and scrambled it to make a type specimen book.",
            titleImg: project_image_2
        },
        {
            title: "Project Veneer by Jabba fett ",
            short_description: "Lorem Ipsum is simply dummy" +
                " text of the printing and typeset" +
                "ting industry. Lorem Ipsum has been the industry's standard dummy t" +
                "ext ever since the 1500s, when an unknown printer took a galley of type" +
                " and scrambled it to make a type specimen book.",
            titleImg: project_image_3
        },
        {
            title: "gatchi dungeon",
            short_description: "Lorem Ipsum is simply dummy" +
                " text of the printing and typeset" +
                "ting industry. Lorem Ipsum has been the industry's standard dummy t" +
                "ext ever since the 1500s, when an unknown printer took a galley of type" +
                " and scrambled it to make a type specimen book.",
            titleImg: project_image_2
        },
        {
            title: "nibiru ",
            short_description: "Lorem Ipsum is simply dummy" +
                " text of the printing and typeset" +
                "ting industry. Lorem Ipsum has been the industry's standard dummy t" +
                "ext ever since the 1500s, when an unknown printer took a galley of type" +
                " and scrambled it to make a type specimen book.",
            titleImg: project_image_3
        },
        {
            title: "Project Veneer by Jabba fett ",
            short_description: "Lorem Ipsum is simply dummy" +
                " text of the printing and typeset" +
                "ting industry. Lorem Ipsum has been the industry's standard dummy t" +
                "ext ever since the 1500s, when an unknown printer took a galley of type" +
                " and scrambled it to make a type specimen book.",
            titleImg: project_image_3
        }
    ],
    gallery: [
        {img: slide1},
        {img: slide2},
        {img: slide2},
        {img: slide2},
        {img: slide2},
        {img: slide2},
        {img: slide3},
        {img: slide4},
        {img: slide4},
        {img: slide4},
        {img: slide5},
        {img: slide6},
        {img: slide7},
        {img: slide7},
        {img: slide7},
        {img: slide8},
        {img: slide8},
        {img: slide8},
        {img: slide8},
        {img: slide8},
        {img: slide8},
        {img: slide8},
        {img: slide8},
        {img: slide8},
        {img: slide8},

    ],
    introSlider: [
        {img: intro},
        {img: intro2},
    ]
}



